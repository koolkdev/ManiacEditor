...
namespace ManiacEditor
{
    ...
    public class Texture2D
    {
        ...
        public void SetParameter(TextureParameterName parameterName, All parameterValue)
        {
            GL.TexParameter(TextureTarget.Texture2D, parameterName, (uint)parameterValue);
        }
        ...
        public void SetImage(Bitmap image)
        {
            ...
            GL.TexImage2D(TextureTarget.Texture2D, 0, PixelInternalFormat.Rgba,
                (int)Width, (int)Height, 0, OpenTK.Graphics.OpenGL.PixelFormat.Bgra, PixelType.UnsignedByte,
                bitmapData.Scan0);
        ...
        public static Texture2D CreateTexture(Bitmap bitmap)
        {
            var texture = new Texture2D();
            texture.Create();
            texture.Bind();
            texture.SetParameter(TextureParameterName.TextureMinFilter, All.Linear);
            texture.SetParameter(TextureParameterName.TextureMagFilter, All.Nearest);
            texture.SetParameter(TextureParameterName.TextureWrapS, All.ClampToEdge);
            texture.SetParameter(TextureParameterName.TextureWrapT, All.ClampToEdge);
